...
sse(y::T, ŷ::T) where T<:AbstractVector{<:AbstractVector} = sum(abs2, reduce(vcat, y - ŷ))/length(y)
sse(y::AbstractVector{<:AbstractVector}, ŷ::AbstractVector) = sum(abs2, reduce(vcat, y) - ŷ)/length(y)
